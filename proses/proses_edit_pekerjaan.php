...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $rasio             = $_POST['rasio'] ?? '';
    $update_pekerjaan  = date('Y-m-d H:i:s');

    // Validasi angka untuk rasio
    if (!is_numeric($rasio)) {
        echo "<script>alert('Rasio harus berupa angka!'); window.history.back();</script>";
        exit;
    }

    // Validasi wajib isi
    if (
        empty($no_ktp) || empty($nama_instansi) || empty($jabatan) || 
        empty($unit_kerja) || empty($status_kerja) || empty($tgl_pengangkatan) || 
        empty($usia_akhir_kerja) || empty($alamat_instansi) || 
